    ...
    try {
      const response = await fetch(import.meta.env.VITE_API_URL + '/register', {
        ...
        body: JSON.stringify({
          ...
          role: form.role, academic_year: form.academic_year, department: form.department,
        }),
      });
      ...
      <div className="w-full max-w-lg relative z-10 animate-fade-in-up">
        <div className="glass-card rounded-3xl p-4 sm:p-6 sm:p-10 space-y-7">
          <div className="text-center space-y-2">
            ...
            <h2 className="text-2xl sm:text-3xl font-black tracking-tight">Create Account</h2>
            <p className="text-gray-500 text-sm">Join the GyanCode learning platform</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-5">
            {step === 1 && (
              <div className="space-y-4 animate-fade-in">
                ...
                {/* Role selector — prominent on Step 1 */}
                ...
                <button type="button" onClick={() => { if (form.name && form.email && form.password) setStep(2); else setError('Fill all fields first.'); }}
            ...
            {step === 2 && (
              <div className="space-y-4 animate-fade-in">
                {/* Show role recap */}
                <div className={`flex items-center gap-3 p-3 rounded-xl border ${form.role === 'Student' ? 'bg-primary/10 border-primary/20' : 'bg-amber-500/10 border-amber-500/20'}`}>
                  <span className="text-xl">{form.role === 'Student' ? '🎓' : '📚'}</span>
                  <div>
                    <p className="text-xs font-bold text-white">{form.role}</p>
                    <p className="text-[10px] text-gray-500">
                      {form.role === 'Student' ? 'Joining to learn & solve challenges' : 'Joining to create & manage assignments'}
                    </p>
                  </div>
                  <button type="button" onClick={() => setStep(1)} className="ml-auto text-[10px] text-primary-light hover:underline">Change</button>
                </div>
